"""Unit tests for scan-worker app/main.py.
...
class TestOnWorkerDone:
    ...
    def test_logs_error_when_task_raises(self):
        ...
        assert "crashed" in call_kwargs[0][0].lower() or "crashed" in str(call_kwargs)
        # error= kwarg carries the stringified exception
...
class TestLifespanStartupFailure:
    ...
    async def test_lifespan_succeeds_when_worker_stays_alive(self):
        ...
        with (
            ...
            # __aenter__ must complete without raising even though run_worker
            # never returns (it will time out after 10 s in real code, but we
            # patch wait_for to return TimeoutError instantly).
            with patch("asyncio.wait_for", side_effect=asyncio.TimeoutError):
                # Should not raise
                await ctx.__aenter__()

            # Cleanup — signal shutdown and exit the context manager
            shutdown_event.set()
            try:
                await ctx.__aexit__(None, None, None)
            except Exception:
                pass  # task cancellation during cleanup is acceptable

    @pytest.mark.asyncio
    async def test_done_callback_is_registered(self):
        """lifespan must call add_done_callback on the worker task."""
        shutdown_event = asyncio.Event()
        ...
        with (
            ...
            from app.main import lifespan, _on_worker_done
            from fastapi import FastAPI
            ...
            # The done-callback list should contain _on_worker_done
            callbacks = [str(cb) for cb in (task._callbacks if hasattr(task, "_callbacks") else [])]
            # We verify indirectly: the task must not be done (worker alive)
