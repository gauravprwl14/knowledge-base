"""
Dedup Worker — FastAPI health & metrics endpoint.
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Dedup worker task started")
    yield
    if _worker_task:
        _worker_task.cancel()
        try:
            ...
            pass
    logger.info("Dedup worker task stopped")


app = FastAPI(
    title="KMS Dedup Worker",
    version="1.0.0",
    docs_url="/docs",
    lifespan=lifespan,
)
...
async def health() -> JSONResponse:
    """Return basic liveness information for the dedup-worker service.
