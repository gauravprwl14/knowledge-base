...
describe('Bm25Service', () => {
  ...
  describe('search() — mock mode', () => {
    ...
    it('should return results sorted by score descending', async () => {
      const results = await service.search('rag pipeline architecture', userId, 10);
    ...
    it('should score the best matching document first', async () => {
      ...
      const results = await service.search('RAG pipeline architecture', userId, 5);

      expect(results[0].id).toBe('mock-chunk-001');
    });

    it('should assign scores in range (0, 1] — never exceeding 0.95', async () => {
      const results = await service.search('RAG BM25 NestJS Neo4j embedding', userId, 5);
    ...
    it('should accept optional sourceIds parameter without error', async () => {
      const results = await service.search('RAG', userId, 5, ['src-001', 'src-002']);
