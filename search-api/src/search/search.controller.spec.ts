...
function makeRequestDto(
  overrides: Partial<SearchRequestDto> = {},
): SearchRequestDto {
...
function makeResponse(
  overrides: Partial<SearchResponseDto> = {},
): SearchResponseDto {
  return {
    results: [],
    total: 0,
...
describe("SearchController", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getLoggerToken(SearchController.name),
          useValue: mockLogger,
        },
      ],
    }).compile();
  ...
  describe("search()", () => {
    ...
    it("should throw HttpException(400) when x-user-id header is absent (undefined)", async () => {
      ...
      await expect(
        controller.search(dto, undefined as unknown as string),
      ).rejects.toThrow(
        new HttpException(
    ...
    it("should propagate HttpException thrown by SearchService", async () => {
      ...
      await expect(controller.search(dto, "user-abc")).rejects.toThrow(
        serviceError,
      );
    });
