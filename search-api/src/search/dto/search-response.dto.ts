...
export class SearchResult {
  @ApiProperty({
    description: "Unique chunk ID (from kms_chunks table or mock)",
  })
  id: string;
  ...
  @ApiPropertyOptional({
    description:
      "Start timestamp in seconds — populated for voice transcript chunks",
  })
  startSecs?: number;

  @ApiPropertyOptional({
    description:
      'Source type of the parent file (e.g. "google_drive", "local", "obsidian")',
  })
  sourceType?: string;
}
...
export class SearchResponseDto {
  ...
  @ApiProperty({
    description: "Wall-clock time taken to execute the search, in milliseconds",
  })
  took: number;
}
